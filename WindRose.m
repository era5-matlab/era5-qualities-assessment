...
LineWidth               = 1.2;
EdgeColor               = 'normal';
legendpos               = 'southoutside';
colorbarpos             = 'eastoutside';
legendori               = 'vertical';  %  vertical  horizontal
analysis                = false;
gap                     = 0;
...
if ~analysis
    ...
    title(TitleString,'color',TitleColor,'fontweight',titlefontweight,'fontname',titlefontname,'fontsize',12,'fontangle',titlefontangle);         % Display a title
    ...
    if LegendType==2                                                           % If legend type is box:
        leyenda = CreateLegend(round(vwinds,2),lablegend,legendvariable,inverse);       % Create a legend cell string
    ...
    elseif LegendType==1                                                       % If legend type is colorbar
        if exist('clim','builtin') || exist('clim','file')
            clim(axs,[round(vwinds(1),2) round(vwinds(end),2)]);                                 % Set colorbar limits
        else
            caxis(axs,[round(vwinds(1),2) round(vwinds(end),2)]);                                % Set colorbar limits
        end
        colormap(axs,interp1(vwinds,color,linspace(min(vwinds),max(vwinds),256))); % set colorbar colours (colormap)
        %c = colorbar('YTick',round(vwinds,2),'fontname',legendfontname,'location',colorbarpos);                % The values shown in the colorbar are the intenisites.
        c = colorbar('YTick',round(vwinds,2),'fontname',legendfontname,'location','southoutside');
        if isempty(lablegend); lablegend = ''; end                             % Ensure a string is available
        set(c, 'Position', [0.1, 0.15, 0.3, 0.02]);  % Adjust position and size (example)
